...
int SendDataPacketThroughLinkLayer(
  ...
  int ret = 0;
  // This is datalayer session id will increment for every data layer
  // transaction
  ...
  if (data_payload_length > 255) {
    DATA_LAYER_PACKET_EXTEND* data_packet =
        (DATA_LAYER_PACKET_EXTEND*)DataPacket;
  ...
  snprintf(key, sizeof(key), PROT_SESSION_KEY, fru_id);
  ...
  for (PackageIndex = 0; PackageIndex <= totalPackageCount; PackageIndex++) {
    ...
    do {
      ...
      ReSendNeeded = 1; // ReSendNeeded need to set, if when AckFlag send
                        // success ReSendNeeded become 0.

      if (package_retry_count) {
        MicroSecondDelay(
            200 * 1000, verbose); // delay of 200ms on retrying the same packet.
      }

      ret = SendPacketThroughLinkLayer(
          fd,
          session_id,
          Source,
          ThisPackageLength,
      ...
      if (ret == 0) {
        ...
        if (AckNeeded == 1) {
          do {
            ...
            DEBUG(
                "Get command LinkLayer Ack %s \n",
                (ack_retry_count ? "retry..." : ""));
            ret = LinkLayerReceiveAcknowldgement(
                fd, session_id, LinkLayerAckBuffer, verbose);
            if (ret) {
            ...
            if (((LINK_LAYER_PACKET_ACK_MASTER*)LinkLayerAckBuffer)
                    ->Length && // Length should be valid
                ((LINK_LAYER_PACKET_ACK_MASTER*)LinkLayerAckBuffer)->AckFlag ==
                    1) { // ACK=0x1, NoACK=0x2
              ReSendNeeded = 0;
              break;
            } else {
...
int SendPacketThroughLinkLayer(
  ...
  LINK_LAYER_PACKET_MASTER* LinkLayerPackage =
      (LINK_LAYER_PACKET_MASTER*)TempBuffer;
  ...
  LinkLayerPackage->Length = (unsigned char)BufferLength +
      2; // add checksum 1Byte & PackageIndex 1Byte
  ...
  LinkLayerPackage->PayLoad[Index] =
      CalculateCheckSum(TempBuffer, sizeof(LINK_LAYER_PACKET_MASTER) - 1);
  if (LinkLayerPackage->Length != 0) {
    DEBUG(
        "Send LinkLayer SubPackage : %x\n", LinkLayerPackage->SubPackageIndex);
    PrintBuffer(TempBuffer, LinkLayerPackage->Length + 2, verbose);
  }
    
  ret = SmbusWriteBlockExecute(
      fd,
      TempBuffer[0],
      LinkLayerPackage->Length,
...
int DataLayerReceiveFromPlatFire(
  ...
  LINK_LAYER_PACKET_ACK_MASTER* LinkLayerPkgBuffer =
      (LINK_LAYER_PACKET_ACK_MASTER*)RecieveBuffer;
  int ret = -1;
  int Start = 0;
  char key[128] = {0};
  char value[128] = {0};
  ...
  do {
    MicroSecondDelay(
        ...
        verbose); 
    ret =
        LinkLayerReceiveAcknowldgement(fd, session_id, RecieveBuffer, verbose);
    if (ret) {
      DEBUG("LinkLayerReceiveAcknowldgement fail \n");
      if (retry < 5) {
        retry++;
    ...
    DEBUG(
        "LinkLayerPkgBuffer->AckNeeded = %u...", LinkLayerPkgBuffer->AckNeeded);
    if (LinkLayerPkgBuffer
            ->AckNeeded) { // If AckNeeded bit set, need to send acknowldgement
      DEBUG("sent ACK to platfire \n");
      LinkLayerSendAcknowldgement(
          fd, session_id, LinkLayerPkgBuffer->SubPackageIndex, verbose);
    } else {
    ...
    memcpy(
        RecieveDataBuffer + Start,
        LinkLayerPkgBuffer->PayLoad,
        LINK_LAYER_MAX_RECEIVE_LOAD); 

    if (LinkLayerPkgBuffer->SubPackageIndex ==
        (LinkLayerPkgBuffer->AckFlag - 1)) {
      break;
    }
  } while (1);
  ...
  PrintBuffer(
      RecieveDataBuffer, RecieveDataBuffer[0] /* data layer length*/, verbose);
...
int LinkLayerSendAcknowldgement(
  ...
  LinkLayerAckBuffer.PayLoad[0] = CalculateCheckSum(
      (unsigned char*)&LinkLayerAckBuffer,
      sizeof(LINK_LAYER_PACKET_MASTER) - 1);
  ...
  ret = SmbusWriteBlockExecute(
      fd, TempBuffer[0], Length, (unsigned char*)&TempBuffer[2], verbose);
...
int LinkLayerReceiveAcknowldgement(
  ...
  if (ret) {
    ...
    unsigned char r_session_id =
        ((LINK_LAYER_PACKET_ACK_MASTER*)RecieveBuffer)->Command;
